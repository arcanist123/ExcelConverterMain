...
public class MaterialGroup {
    ...
    public static MaterialGroup createMaterialGroup(Element materialGroupElement) throws IOException {
        ...
        materialGroup.groupName = materialGroupElement.getElementsByTagName(Tags.GROUP_NAME.toString()).item(0).getTextContent();
        
        //get the material group GUID
        materialGroup.groupGUID = materialGroupElement.getElementsByTagName(Tags.GROUP_GUID.toString()).item(0).getTextContent();
        
        //parse the individual materials into  vector of materials
        NodeList materials = materialGroupElement.getElementsByTagName(Tags.MATERIAL.toString());
    ...
    public short writeToXLSX(Sheet mainSheet, short currentRow) {
        ...
        for (int i = 0; i < this.materials.size(); i++){
            currentRow = this.materials.elementAt(i).writeToXLSX(mainSheet, currentRow);
